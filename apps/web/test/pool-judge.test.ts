...
describe('toPoolJudgement', () => {
  ...
  describe('代表表現', () => {
     ...
     * 化けた代表表現を止める（#336 / #340）。
     *
     * 🔴 **小型モデルは canonical を作るとき、元の言葉を化けさせる。**
     * 元にあった漢字が消え、かつ「元に無い漢字」または「元に無いかな／カナの
     * 3文字以上のかたまり」が現れていたら化けとみなす。
     * 下の組はすべて本番の `/discover` で観測したもの。
     */
    describe('🔴 化けた代表表現なら使わない', () => {
      const keeps = (canonical: string, normalized: string) =>
        toPoolJudgement({ ...ok, canonical }, normalized)
      ...
      it.each([
        ...
        ['転職する', '轉属する'],
        ['陶芸で器を作る', '磊象で器を作る'],
        ['海の近くで魚を食べる', '海の载くで鱼を食べる'],
        ['一人で焼肉に行く', '無一で焼煤に行に'],
        ['初対面の人に話しかける', '初連面の人に話すかる'],
        ['大切な人に感謝を伝える', '健たな人に感走を依る'],
        ['好きな作家の全作品を読む', 'きな語書がきなの作品を読べる'],
        ['家系図をたどってみる', '家組図をたられてべる'],
        ['手話を覚える', '日本数字の手話を議える'],
        ['果物狩りに行く', '果物蚈に行に自す'],
        ['洞窟の奥まで歩く', '天気の奥まはに行く'],
        ['献血を10回する', '共同磯理を10回停止する'],
        ['真冬の海で泳ぐ', '玻曆の海で湯す'],
        ['自分の書斎を持つ', '自当の書給を持つる'],
        ['苦手な仕事を一度やり切る', '因にる工作を一次よりわる'],
        // 漢字 → でたらめなかな／別のカタカナ語（#340）
        ['AIで何か作る', 'AIでなぜまを作る'],
        ['新しい友達を作る', 'フレンドを作る'],
      ])('%s → %s は元の文に戻す', (normalized, canonical) => {
        expect(keeps(canonical, normalized)).toMatchObject({ canonical: normalized })
      })

      it('体言止めに動詞を足すだけなら通す（消える漢字が無い）', () => {
        expect(keeps('万里の長城に行く', '万里の長城')).toMatchObject({
          canonical: '万里の長城に行く',
        })
      })

      it('🔴 かなの語尾を足すだけなら通す（消える漢字が無い）', () => {
        expect(keeps('グランピングをする', 'グランピング')).toMatchObject({
          canonical: 'グランピングをする',
        })
      })

      it('語尾だけ書き換えたものは通す（漢字の増減が無い）', () => {
        expect(keeps('富士山に登る', '富士山に登りたい')).toMatchObject({
          canonical: '富士山に登る',
        })
      })

      it('🔴 漢字が減って、増えるかなが1〜2文字なら通す（「頂」→「に登る」）', () => {
        expect(keeps('富士山に登る', '富士山登頂')).toMatchObject({ canonical: '富士山に登る' })
      })

      it('かなの送り違いだけでは止めない（漢字は増減していない）', () => {
        expect(keeps('本を読む', '本を読みたい')).toMatchObject({ canonical: '本を読む' })
      })
    })
  })
