...
**外部 / 公開。** アプリ情報に規約とポリシーの URL を登録してある（2026-09-01、#304）。

| 項目 | 値 |
|---|---|
| プライバシーポリシー | `https://yaritai100list.aiandrox.workers.dev/privacy` |
| 利用規約 | `https://yaritai100list.aiandrox.workers.dev/terms` |

🔴 **ここが「ログイン前に読ませる」の唯一の場所。** Google が Google の画面で出す。
アプリ側にも同意のモーダルを置いていたが、**ここで足りるので廃止した**（#331）。
**この登録を外すと、読ませる機会が無くなる。**

⚠️ **リンク先を消したり、経路を変えたりしないこと。**
公開している OAuth アプリはプライバシーポリシーの提示を求められる。
`/terms` と `/privacy` は SPA の経路なので、**`App.tsx` の `Route` を消すと 404 になる。**
...
## Google AI Studio / Gemini API（取り入れ面の判定。#253 / #342）

取り入れ面に出してよいかの判定に使う。**2026-09-06 に Workers AI（Llama）から移した**
（#336 / #340 / #342）。理由は文字化け — fp8 量子化＋日本語の弱さで canonical を
別字に化けさせる事故が本番で ~18/250 出ていた。検証は #342 のコメント。

| 項目 | 現在値 | 備考 |
|---|---|---|
| コンソール | https://aistudio.google.com | Vertex ではなく AI Studio（API キー1本） |
| GCP プロジェクト | **`yaritai100list`** | AI Studio のキーはプロジェクトに紐づく。**別プロジェクト（`gen-lang-client-*`）に作らない** |
| 有効化する API | **Generative Language API**（`generativelanguage.googleapis.com`） | プロジェクトで有効にしておく |
| モデル | **`gemini-3.5-flash-lite`**（`apps/web/src/pool-judge.ts` の `POOL_JUDGE_MODEL`） | `gemini-2.5-flash-lite` は #342 の検証中に「新規ユーザーに提供しない」で 404 になった |
| キーの名前 | **開発 `pool-judge-development` / 本番 `pool-judge-production`**（AI Studio のラベル） | 値はここに書かない |
| 課金 | **前払い（プリペイド）**。残高が要る | 日本アカウントは前払い固定のことが多い。残高 0 だと `429 RESOURCE_EXHAUSTED "prepayment credits depleted"` |
| コスト | 1件あたり入力 約 800 / 出力 約 25 トークン。12件/時 × 24 = 288件/日 で **月 $1〜3** | 暗黙のプロンプトキャッシュ（同一システムプロンプト）で入力側はさらに下がる |

🔴 **キーは環境ごとに別で発行する**（`docs/console-settings.md` の他の秘密と同じ規律）。
`.dev.vars` に本番キーを降ろさない。変数名はどちらの環境でも `GEMINI_API_KEY`、**値だけ**が違う。

🔴 **モデルは提供終了する。** 落ちても「出さない」として保存はしないので誤判定は焼き付かないが、
**判定が全く進まなくなる。** `pool-judge: ... すべて失敗した` が Sentry に飛ぶ。

⚠️ **`AI` バインディングはもう無い。** `wrangler.jsonc` から消した。
`vitest-pool-workers` がローカル模倣の無いバインディングを CI で要求する問題（旧 #253）も、
これで消えた。

### 経緯: 課金でつまずいた（2026-09-06、#342）

- 最初 `gemini-2.5-flash-lite` を指定 → **新規ユーザーには提供終了**で 404。`gemini-3.5-flash-lite` に
- 次に `429 "Your prepayment credits are depleted"` → **プロジェクトに前払い残高が無かった。**
  `yaritai100list` プロジェクトの Billing で前払いクレジットを購入して解消（Vertex の後払いには切り替えなかった）
- キーが別プロジェクト（`gen-lang-client-*`、請求無効）に作られていないか確認すること

### モデルやプロンプトを変えるとき（#254 / #264 / #342）
...
| `GEMINI_API_KEY` | 取り入れ面の判定（#342。**シークレット。理由は Gemini の節**）。無ければ判定バッチは動かない | `.dev.vars`（開発キー） | `wrangler secret put`（本番キー） | AI Studio |
