...
export const items = sqliteTable(
  'items',
  {
    ...
    /**
     * 自分だけが読むメモ（#294）。**理由・補足・叶えたときのこと。**
     *
     * 🔴 **共有ページにも取り入れ面にも出さない**（`PRODUCT_SPEC.md` §4.4 の表）。
     * 本文（`text`）と扱いが根本的に違う列で、
     * **`hiddenInShare` のような出し分けも持たない**（常に出さない）。
     *
     * ⚠️ **空文字を入れない。** 「書いていない」は NULL 1つで表す。
     * 2通りの「空」があると、画面もテストも両方を気にすることになる。
     *
     * 上限（500文字）は `packages/shared` の Zod。**DB には書かない**
     * （上限の情報源は1箇所。`CLAUDE.md` の不変条件）。
     */
    memo: text('memo'),
