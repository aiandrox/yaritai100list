...
**後始末は完了している**（#1、2026-08-06）。

| 対象 | 状態 |
|---|---|
| 旧 GCP プロジェクト | **削除済み**（完全消滅は 2026-09-05 頃） |
| `aiandrox/yaritaikoto100` | **アーカイブ済み**（public のまま） |
| `aiandrox/yaritaikoto100_be` | **アーカイブ済み**（public のまま） |
| Vercel `yaritaikoto100.vercel.app` | **削除済み。** 案内ページは置かなかった |
| ローカル `~/private/development/yaritaikoto100/` | **削除済み**（`_be` / `_fe` / `_fe2`、約 1.0GB） |

### 旧リポジトリは public のままアーカイブした

**git 履歴に `GOOGLE_CLIENT_SECRET` / `NEXTAUTH_SECRET` が残っている**が、
旧 GCP プロジェクトごと削除しているため**鍵は無効化済み**。
アプリ自体も消えているので `NEXTAUTH_SECRET` も意味を持たない。

**アーカイブしたリポジトリは読み取り専用になる。**
後から履歴を書き換えたくなっても、先にアーカイブを解除する必要がある。

### 旧利用者への対応: 何も置かなかった

Vercel のプロジェクトごと削除したので、`yaritaikoto100.vercel.app` は 404 になる。
新サービスへの案内ページは置いていない。

**旧アプリの利用者データは各利用者のブラウザの localStorage にのみ存在し、
ドメインが変わるため移行不能**（`MEMO.md` §2）。
案内を置いても「データは引き継げません」としか言えないため、置かない判断をした。

### ローカルの破棄で捨てたもの

削除前に確認した内容の記録（もう復元できないため）。

| ディレクトリ | リモート | 未 push のコミット | 未コミットの変更 |
|---|---|---|---|
| `_be` | `yaritaikoto100_be` | 2件（`logout` / ヘルスチェック追加） | なし |
| `_fe2` | `yaritaikoto100` | 2件（`logout wip` / `env更新`） | 5ファイル |
| `_fe` | **なし**（ローカルのみ） | 3件すべて | 4ファイル |

**push せずに捨てた。** 旧 BE は「捨てる」が決定事項（`TECH_STACK.md` §11）で、
棚卸しは `docs/legacy-spec.md` に保存済み。認証は Better Auth で作り直すため WIP の logout 実装に価値がなく、
`env更新` というコミットを凍結直前のリポジトリに足す動きも避けたかった。

トップレベルにあった `PRODUCT_SPEC.md` / `TECH_STACK.md` / `SPEC.md` は、
このリポジトリの内容の方が新しいことを差分で確認済み（`SPEC.md` は `docs/legacy-spec.md` と完全一致）。
