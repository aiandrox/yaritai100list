 ...
 * 1回のバッチで判定する本文の数。
 *
 * 🔴 **Neurons の1日の枠から決めている**（2026-08-10 に実測して 15 から下げた）。
 * 1時間ごとに動くので **× 24 が1日の消費**になる。
 *
 * | | 1件あたり | 1日（× 24） |
 * |---|---|---|
 * | #253 の短いプロンプト・15件 | 約 20 neurons | 7,200 … 枠内 |
 * | #264 の長いプロンプト・15件 | **約 48 neurons** | **17,280 … 🔴 枠を超える** |
 * | #264 の長いプロンプト・6件 | 約 48 neurons | 6,912 … 枠内 |
 *
 * ⚠️ **無料枠は 10,000 neurons/日。** 超えると `AiError 4006` で
 * **その日はもう1件も判定できない**（実際に踏んだ）。
 * 判定が止まってもプールは古い判定で埋まったままなので画面は壊れないが、
 * **新しく書かれた本文がいつまでも出てこない。**
 *
 * 🔴 **プロンプトを長くしたら、ここも見直すこと。** 精度と件数は
 * 同じ枠を取り合っている。**精度を取って件数を半分以下にした**のが今の値。
 *
 * ⚠️ サブリクエストの上限（50/実行）にも当たってはいけないが、
 * **先に効くのは Neurons の方**（6件なら 13 サブリクエスト）。
 */
export const POOL_JUDGE_BATCH_SIZE = 6
