...
export interface SignInBenefit {
  ...
  title: string
  /**
   * いまの状態。**読み手が知らないことを言うときだけ書く。**
   *
   * 🔴 「見せられません」のような**見出しの裏返しは書かない。**
   * 全部の項目に付けると、読む量が倍になるのに増える情報はゼロになる。
   */
  without?: string
  /** ログインした後。**具体的に何ができるか** */
  with: string
  ...
  return [
    {
      ...
      title: 'リストが消えない',
      without: 'いま書いているリストは、このブラウザにしかありません。履歴を消すと無くなります',
      with: 'サーバーに保存されます。別の端末でも同じリストを編集できます',
    },
    {
      title: '「やった」印を付けられる',
      with: 'やった項目にチェックを付けて、叶えた日付を残せます',
    },
    {
      title: `リストを${String(LISTS_PER_USER_MAX)}つまで持てる`,
      without: 'いまは1つだけです',
      with: `「${String(now.getFullYear())}年に」「一生のうちに」のように使い分けられます`,
    },
    {
      title: '人に見せられる',
      with: '共有リンクを発行すると、他の人がリストを見られます',
    },
    {
      // 画像とマークダウンをまとめる。**どちらも「持ち出す」こと**で、
      // 分けても利点が2つあるようには読めない
      title: '画像やマークダウンで書き出せる',
      with: `やりたいこと${String(ITEMS_PER_LIST_MAX)}個を1枚の画像にして SNS に貼れます。ブログ用にマークダウンでも書き出せます`,
    },
  ]
}
