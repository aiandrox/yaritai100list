...
| 1回の判定 | **約 48 Neurons**（入力 26,668/M・出力 204,805/M） |
| 1日に判定できる数 | **約 200 件** |
| バッチの設定 | 1時間ごと × **6件** = 144件/日（約 6,900 Neurons） |
...
🔴 **精度と件数は同じ枠を取り合っている。**
#264 でプロンプトを長くした（例を並べた）ぶん、1件あたりが **20 → 48 Neurons** になり、
**1日に判定できる数が 500 → 200 件に減った。**
だから `POOL_JUDGE_BATCH_SIZE` を 15 から **6** に下げてある。

⚠️ **プロンプトを長くしたら、必ずバッチの件数を見直すこと。**
見直さないと、14時間で枠を焼いて残りの10時間が無駄になる。
